...
RESULTS = [
 ...
 dict(sid="r5", num="4.5", fig="fig_step5_noise_association.png", fign=5,
   title="Detecting statistical dependence under dropout with four association measures",
   samples="<b>4000 cells per condition</b>; snapshot at t = 600 min; five capture efficiencies &times; six independent noise draws each. HSIC and dCor use a reproducible 1200-cell subsample per draw. The clean snapshot and all summary values are saved in <code>data/cache_step5.npz</code>.",
   caption="Dependence between p53 and MDM2 mRNA estimated by Pearson correlation, mutual information (kNN estimator), the normalized Hilbert-Schmidt Independence Criterion (HSIC) and distance correlation (dCor), as a function of simple binomial capture efficiency in the closed loop versus Nutlin-3. Error bars show the standard deviation across six dropout realizations. Each panel has its own metric-specific y-scale, so heights should be compared between conditions or capture efficiencies within a panel, not numerically across methods.",
   interp=[
    "All four measures separate the closed loop from Nutlin when capture is high and lose signal as dropout increases. At full capture, Pearson = 0.431 and dCor = 0.414 in the closed loop, compared with 0.019 and 0.045 under Nutlin. Their similar closed-loop behavior indicates that dCor does not reveal a large additional nonlinear component here.",
    "The full-capture MI and normalized HSIC values are 0.103 and 0.094 in the closed loop. These smaller numbers do not by themselves mean weaker biological dependence: MI is measured in nats and normalized HSIC has a different scale, so their raw magnitudes cannot be compared with a correlation coefficient. What matters in this figure is their separation from the corresponding Nutlin/no-dependence curve and how rapidly that separation disappears with dropout. Formal cross-method comparison uses permutation z-scores in Figure 7, where the kNN MI estimator indeed shows lower detection power.",
 ...
 dict(sid="r6", num="4.6", fig="fig_step6_splatter_methods.png", fign=6,
   ...
   caption="The same detection task under the Splatter technical-noise model (Zappia et al., 2017)&mdash;log-normal library size, negative-binomial overdispersion, and expression-dependent logistic dropout&mdash;evaluated with four dependence measures including distance correlation (dCor).",
   interp=[
 ...
 dict(sid="r7", num=None, fig="fig_step6_permutation_z.png", fign=7,
   ...
   interp=[
    "In the closed loop the true dependence is strongly significant for Pearson (z &approx; 6), HSIC (&approx; 12) and distance correlation (&approx; 12); mutual information is the exception (z &approx; 0.7), because the kNN MI estimator is a weak, high-variance detector on discrete count data (Section 4.5).",
    "Under Nutlin-3 the two transcripts are genuinely independent, so every bar there <i>should</i> be zero. Instead Pearson (z &approx; 3.5), HSIC (&approx; 3.6) and dCor (&approx; 5.3) are all significant <i>false positives</i>, produced by the shared per-cell library-size factor; only mutual information stays near zero (z &approx; 0.7). The correct reading is therefore not that any one statistic is intrinsically 'robust' &mdash; the powerful measures all detect the library-size artefact, and MI only escapes because of its low power. The real remedy is explicit <b>library-size normalization</b>, which we apply to the experimental data in Section 4.7."]),
 ...
 dict(sid="r10", num="4.9", fig="fig_step9_directional.png", fign=13,
   ...
   caption="(A) lagged cross-correlation and (B) lagged distance correlation of the detrended p53/MDM2 mRNA "
           ...
           "correlations from the synthetic asynchronous Nutlin snapshot before and after Splatter technical noise; "
...
protein-level common driver available in the synthetic model. The noisy Figure 13E bars apply the
