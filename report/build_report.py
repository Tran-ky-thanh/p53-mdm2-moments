...
RESULTS = [
 ...
 dict(sid="r5", num="4.5", fig="fig_step5_noise_association.png", fign=5,
   title="Detecting statistical dependence under dropout with five association measures",
   ...
   caption="Dependence between p53 and MDM2 mRNA estimated by Pearson correlation, Spearman correlation, mutual information (kNN estimator), the normalized Hilbert-Schmidt Independence Criterion (HSIC) and distance correlation (dCor), as a function of simple binomial capture efficiency in the closed loop versus Nutlin-3. Error bars show the standard deviation across six dropout realizations. Each panel has its own metric-specific y-scale, so heights should be compared between conditions or capture efficiencies within a panel, not numerically across methods.",
   interp=[
    "All five measures separate the closed loop from Nutlin when capture is high and lose signal as dropout increases. At full capture, Pearson = 0.431, Spearman = 0.433 and dCor = 0.414 in the closed loop, compared with 0.019, 0.016 and 0.045 under Nutlin. Their similar closed-loop values indicate that the detectable dependence is predominantly monotone and close to linear; dCor does not reveal a large additional nonlinear component here.",
